"""
xbrl.py — Parse EDGAR-extracted XBRL instance documents into tidy tables.

This module consumes the ``xbrl_instance_url`` produced by
``core.fetch_filings_for_ciks`` (or by ``daily_index.enrich_record`` on the
date-driven path) and turns each extracted instance document (e.g.
"aapl-20260430_htm.xml") into a clean, long-format pandas DataFrame with **one
row per reported fact** — value, unit, period, and any dimensional (segment)
breakdown resolved into plain columns.

``entity_facts()`` and ``security_facts()`` at the bottom collapse the DEI cover
page to entity and security level; :mod:`finalized.cover` builds on them to
produce the stored ``entity_cover`` / ``security_cover`` tables.
...
Usage
-----
    from .core import fetch_filings_for_ciks
    from .xbrl import parse_filings
...
def security_share_dates(facts: pd.DataFrame) -> dict:
    """
    Map each security to the date its share count was actually measured.

    A cover-page share count is an *instant* fact, and its instant is not the
    filing date: the count is taken shortly before filing, and how much before
    varies enormously by form — measured across this mirror, a 10-Q's count is
    ~6 days old at filing, a 10-K's ~9, but a 20-F's is **104 days** old and a
    20-F/A's 162.  So "shares as of the filing date" is wrong by a quarter or
    more for foreign private issuers, and the instant has to travel with the
    number rather than be re-derived from the filing.

    Keys are the raw axis member (``us-gaap:CommonClassAMember``), or ``None``
    for the un-dimensioned single-security line — the same key
    :func:`security_facts` reports as ``security_member``, so the two join
    directly.

    Expects the facts of ONE filing (which is how cover.py drives it).
    """
    dei = _dei_frame(facts)
    if dei.empty or "period_instant" not in dei.columns:
        return {}
    axes = _discover_security_axes(dei)
    shares = dei[dei["concept"] == "EntityCommonStockSharesOutstanding"]
    out: dict = {}
    for _, row in shares.iterrows():
        instant = row.get("period_instant")
        if pd.notna(instant):
            # Keep the first instant seen for a member; a filing that restates
            # one class twice reports the same instant both times.
            out.setdefault(_security_member(row["dimensions"], axes), instant)
    return out
