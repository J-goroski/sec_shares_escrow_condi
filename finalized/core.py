...
UA = {"User-Agent": "jared.goroski@gmail.com"}
...
class FilingRecord:
    ...
    filing_detail_url  — URL to the human-readable filing detail (index) page,
                         "<accession>-index.html"
    submission_txt_url — URL to the complete submission text file (full raw
                         SGML submission), "<accession>.txt"
    xbrl_instance_url  — URL to the EDGAR-extracted XBRL instance document
                         (e.g. "aapl-20260430_htm.xml"), or None when the
                         filing has no inline XBRL to extract
    is_xbrl            — True when the filing includes inline XBRL (iXBRL)
                         tagging, which enables structured data extraction
    """
    cik: str
    entity_name: str
    ticker: str
    ...
    xbrl_instance_url: Optional[str]
...
def _build_urls(cik_int: int, accession: str, primary_doc: str) -> tuple[str, str, str, str]:
    # Accession numbers like "0000320193-25-000079" map to folder "000032019325000079"
    folder = accession.replace("-", "")
    base   = f"{ARCHIVES_BASE}/{cik_int}/{folder}"
    filing_url = f"{base}/{primary_doc}"
    index_url  = f"{base}/"
    # Filing detail (index) page — human-readable page listing every document
    # in the submission, named "<accession>-index.html" (e.g.
    # ".../0000320193-26-000011-index.html")
    filing_detail_url  = f"{base}/{accession}-index.html"
    # Complete submission text file — the full raw SGML submission, named
    # "<accession>.txt" (e.g. ".../0000320193-26-000011.txt")
    submission_txt_url = f"{base}/{accession}.txt"
    return filing_url, index_url, filing_detail_url, submission_txt_url


def _build_xbrl_instance_url(
    cik_int: int, accession: str, primary_doc: str, is_inline_xbrl: bool
) -> str | None:
    """
    URL to the EDGAR-extracted XBRL instance document, or None if unavailable.

    EDGAR generates this file only for *inline* XBRL (iXBRL) filings, by taking
    the primary HTML document and replacing the extension dot with an
    underscore before appending ".xml":

        aapl-20260430.htm  ->  aapl-20260430_htm.xml

    Older-style (non-inline) XBRL filings ship their instance as a separately
    named EX-101.INS exhibit that cannot be derived from the primary document,
    so we return None for those rather than guess a wrong URL.
    """
    if not is_inline_xbrl or not primary_doc:
        return None
    stem, dot, ext = primary_doc.rpartition(".")
    if not dot or ext.lower() not in ("htm", "html"):
        return None
    folder = accession.replace("-", "")
    return f"{ARCHIVES_BASE}/{cik_int}/{folder}/{stem}_{ext}.xml"
...
def _parse_recent_filings(
    ...
    for i, form in enumerate(forms):
        ...
        filing_url, index_url, filing_detail_url, submission_txt_url = _build_urls(
            cik_int, accession, primary_doc,
        )
        is_inline_xbrl = bool(_get(is_inline_xbrls, i))
        xbrl_instance_url = _build_xbrl_instance_url(
            cik_int, accession, primary_doc, is_inline_xbrl,
        )
